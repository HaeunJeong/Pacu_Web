...
def single_frame_contours(frame, equalize=True):
    ...
    hulls = [hull for hull,area in zip(hulls,areas) if area > lower_area_limit and area < upper_area_limit]
    ...
    if len(centroids):
        ...
        return center_contour, circ_score < circ_score_thresh
    else:
...
def main():
    ...
    contours = np.array(all_frame_contours(eye_data))
    np.save('{}_contours'.format(basename), contours)

    if '-write' in sys.argv:
        print('Annotating data...')
        annotated_data = annotate_eye(eye_data, contours[:,0])
        print('Saving annotated data...')
        tif.imsave('{}_annotated.tif'.format(basename), annotated_data)
        
        # annotated_data를 input으로 넣으면 합친게 나올거고,
        # 그냥 eye_data? 를 넣으면 원본 영상만 나올거고
    ...
    if '-avi' in sys.argv:
        ...
        depth, height, width = eye_data.shape
        out = cv2.VideoWriter('{}_video.avi'.format(basename),-1, 20.0, (width,height))    
        ...
        for frame in eye_data:
            ...
            # i = np.random.randint(0, 255, (480,640,3)).astype('uint8') 
            # out.write(i)
            # print(i)
    ...
    if '-jpg' in sys.argv:
        ...
        for i, frame in enumerate(sample):
            scipy.misc.imsave('./media/pacu/{}.jpg'.format(i), frame)
